...
function searchWord(word, str) {
  let filterArr = str.split(" ").filter(element => element.toLowerCase() === word);
...
function searchWord(word = '', str = '') {
  ...
  if (word === '' || str === '') {
    return "Error: you are missing a word to search for or a text to search through";
  } else {

    /* For returning number of occurences  
    let matches = str.match(regex);

    return matches ? matches.length : 0;
    */
   
    // For returning highlighted instances

    return str.replace(regex, toHighlighted(word))
  }
}

function toHighlighted(word) {
  return `**${word.toUpperCase()}**`
}

console.log(searchWord('qui', text))
console.log(searchWord(text));
console.log(searchWord('check'));
